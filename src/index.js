...
socketIo.on('connection', socket => {
  ...
  socket.on('join', ({ username, room }, callback) => {
    ...
    socket.emit('message', generateMessage('システム', 'ようこそ！')); //emits event to a particular connections

    socket.broadcast.to(user.room).emit('message', generateMessage('システム', `${user.username} が参加しました。`)
  ...
  socket.on('sendMessage', (sentMessage, callback) => {
    ...
    if (filter.isProfane(sentMessage)) {
      ...
      return callback('残念！そのメッセージは禁止ワードだにょーんｗｗｗｗｗｗ');
    }
  ...
  socket.on('disconnect', () => {
    ...
    if (user) {
      ...
      socketIo.to(room).emit('message',generateMessage('システム', `${username} が死んだ...`));
